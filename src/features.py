...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Crea features de ingeniería con fundamento físico.

    Parameters:
        df: pd.DataFrame DataFrame limpio de la fase de cleaning.

    Returns:
        pd.DataFrame: DataFrame con 3 columnas adicionales.
    """
    logger.info("Construyendo features de ingeniería")
    df = df.copy()

    # Feature 1: Delta térmico
    # Un proceso saludable mantiene una diferencia estable entre
    # temperatura de proceso y temperatura ambiente (~10K).
